...
func User_GetByUUID(uuid any) (app_models.Users, error) {
	...
	err := AppDB.Where("uuid = ?", uuid).First(&userbyid)
	if err.Error != nil {
		log.Println("Error getting user by ID:", uuid, ErrUserNotFound)
...
func User_UpdateAuth(uuid any, isauth bool) error {
	err := AppDB.Model(&app_models.Users{}).Where("uuid = ?", uuid).Update("is_auth", isauth)
	if err.Error != nil {
		return err.Error
	}
	return nil
...
func User_UpdateRole(uuid any, role string) error {
	err := AppDB.Model(&app_models.Users{}).Where("uuid = ?", uuid).Update("role", role)
	if err.Error != nil {
		return err.Error
	}
	return nil
...
func User_UpdateOrg(uuid any, orgname string) error {
	err := AppDB.Model(&app_models.Users{}).Where("uuid = ?", uuid).Update("org", orgname)
	if err.Error != nil {
		return err.Error
	}
	return nil
...
func User_SetNewPassword(uuid any, password string) error {
	err := AppDB.Model(&app_models.Users{}).Where("uuid = ?", uuid).Update("password", password)
	if err.Error != nil {
		return err.Error
	}
	return nil
